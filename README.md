...
The solid black ink circle combines a filled core with three overlapping marker passes in `#000000`, generated with the article’s drawing geometry. The SVG master lives in `public/identity/ink-circle.svg`. Favicon SVG/ICO/PNG files, the Apple touch icon and `public/social-preview.webp` are linked in `index.html`.
